...
If your resource uses `idField` (custom ID like `slug`, `sku`, `code`) or permission filters with org-scoping, implement `getOne`:

```typescript
interface CrudRepository<TDoc> {
  // ... required methods above

  /** Find single doc by compound filter — used by AccessControl for idField + org scoping */
