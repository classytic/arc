...
```typescript
// Core (defineResource + permissions + errors)
import { defineResource, allowPublic } from '@classytic/arc';

// Adapters ship from their kits, not arc (kit owns the driver dep)
...
import { guard, transform, pipe } from '@classytic/arc/pipeline';

// Middleware (multipartBody, named middleware)
import { multipartBody, middleware } from '@classytic/arc/middleware';

// Request context (AsyncLocalStorage)
import { requestContext } from '@classytic/arc/context';

// Logger (debug / warning surface)
...
| `@classytic/arc/schemas` | typebox | Yes | TypeBox v1 schema helpers |
...
// BAD — pre-2.1 pattern that pulled Redis into the events barrel
