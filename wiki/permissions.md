...
"Compare the request body against the auth user" (`body.authorId`, xAPI actor IFI) is a recurring ask, and arc deliberately ships no body-vs-auth `PermissionCheck`: a `PermissionCheck` answers "can this caller do this action?", not "does this body shape match scope?". Pick by where the field structurally lives.

**Flat field on the document → stamp it.** `fieldRules: { authorId: { systemManaged: true } }` strips the client value at the sanitizer; a `beforeCreate` hook sets it from scope (`ctx.data.authorId = getUserId(ctx.user)`). The client cannot spoof, because the field never reaches the DB from user input — and under the default `onFieldWriteDenied: 'reject'` the attempt fails loudly.

**Nested vendor envelope → validate it.** When the field lives inside a shape you cannot strip without breaking it (`statement.actor.account.name`), compare in `beforeCreate` and throw a domain error naming the item index. Typed against `TDoc`, composes with other validations, no stringly-typed dot-paths.

**Why no `requireActorMatchesAuth()`:** it would overlap both — `fieldRules` already stamps, `beforeCreate` already validates. A permission helper that *looks* like an auth gate but performs data validation is a category error: consumers reach for it when they should be stamping. Fewer, deeper primitives that compose.

**`requireOwnership` is read-side** — it checks the LOADED doc's owner field (`update` / `delete` / row-filter `list`). It never inspects the inbound body and is not a substitute for stamping. See [[core]] for the sanitizer pipeline.
