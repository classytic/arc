...
| @classytic/primitives | >=0.21.0 | **Yes** | Canonical event types (`EventMeta`, `DomainEvent`, `EventTransport` — `subscribe` optional as of 0.14) + outbox contract; **0.15 floor**: `/canonical` + `/retention` subpaths the cleanup framework imports (absent in 0.14 → `ERR_PACKAGE_PATH_NOT_EXPORTED`); **0.18 floor**: `OutboxStore.transactionalSave` — arc's repository outbox adapter declares the capability flag, and it is absent from published 0.16.0 (arc's source needed a version above its own declared floor; caught when a clean install downgraded the cp-dist copy); **0.20 floor**: the outbox OPERATOR surface — `OutboxStatus`, `OutboxStore.requeue` / `countByStatus` / `oldestPendingAgeMs`, which `repositoryAsOutboxStore` implements and narrows via `Required<Pick<…>>`. Absent from published 0.18.0, so the same class of skew as the 0.18 row: `check:peer-skew` cannot catch it, because it only proves the devDep covers the floor — not that source stays within it. ; **0.21 floor**: `@classytic/primitives/environment` (`classifyEnv` / `isProductionEnv`) — arc reads it at every NODE_ENV behaviour site and the CLI scaffold imports it, and the subpath does not exist below 0.21.0. |
| @classytic/repo-core | >=0.23.0 | **Yes** | `RepositoryLike`, adapter contract (`/adapter`), canonical pagination / tenant / errors / schema-generator contracts; **0.17 floor**: `/cleanup` step contract (`CleanupStep`) |
