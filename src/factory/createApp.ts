...
export async function createApp(options: CreateAppOptions): Promise<FastifyInstance> {
  ...
  const useArcAuth = authConfig !== false && !authConfig?.plugin;
  if (useArcAuth && !options.jwtSecret) {
    throw new Error('createApp: jwtSecret is required when Arc auth is enabled');
  }
  ...
  // ============================================
  // 8. REGISTER AUTHENTICATION (Arc or custom)
  // ============================================

  if (config.auth !== false) {
    if (config.auth?.plugin) {
      await config.auth.plugin(fastify);
