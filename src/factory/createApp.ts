...
export async function createApp(
  ...
  const hasCustomPlugin =
    ...
    "plugin" in authConfig &&
    authConfig.plugin;
  const hasCustomAuthenticator =
    typeof authConfig === "object" && "authenticate" in authConfig;
  const jwtSecret =
    typeof authConfig === "object" && "jwt" in authConfig
  ...
  if (!isAuthDisabled) {
    if (hasCustomPlugin) {
      ...
      const pluginFn = (
        authConfig as { plugin: (fastify: any) => Promise<void> }
      ).plugin;
