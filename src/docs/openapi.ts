...
function generateResourcePaths(
  ...
  // No early-return guard: each section below self-gates on its own source
  // (`disableDefaultRoutes`, empty `customRoutes`/`actions`/`aggregations`)
  // and naturally contributes zero paths when empty. A combined early-return
  // is dead weight that drops resources whenever a new path-source is added
  // without being threaded into the guard — see 2.13 aggregations regression.
  ...
  // v2.13 — Generate `GET /aggregations/:name` per declared aggregation.
  // Each aggregation has its own path entry (not a single dispatch
  // endpoint like actions) so the response schema can be derived from
  // the specific groupBy + measures set.
 ...
 */
function appendAggregationPaths(
  paths: Record<string, PathItem>,
  resource: RegistryEntry,
  basePath: string,
  additionalSecurity: Array<Record<string, string[]>>,
        ...
        parameters: [
          {
            ...
            in: "query",
            required: false,
            // Open-shape querystring — frontends add filters as needed.
