...
| [2.33](changelog/v2.md#2330) | **Write verbs** + **transactional write envelope** (`transactional: true` — tx-bound repo into verbs, transient-conflict retry, hooks once, boot-fatal without `withTransaction`; ⚠ repo-core floor >=0.24.0, primitives >=0.23.0) — bind a CRUD slot to a domain command, pipeline intact; reachability is boot-fatal. Outbox published NOTHING through its default transport (fail-open facade → raw `EventTransport`); `onHandlerError:'throw'`; single-node topology declarable; relay id per instance. Also: ⚠ `NODE_ENV=prod` disclosed stack traces + raw 500 messages and dropped `Secure` cookies. Silent input drop closed on two paths (`strictFilterFields`, `onImmutableWrite`, both opt-in). |
