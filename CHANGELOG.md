...
## 2.10.2 — fix: silent pagination in audit/outbox adapters + onSend race closures

**Critical bug fixes — 2.10.0 and 2.10.1 are deprecated on npm.**

### Plugin onSend → earlier-hook migration (closes ERR_HTTP_HEADERS_SENT races)
...
rejections but didn't close the race window. 2.9.3 moved `caching.ts`
from `onSend` to `preSerialization`, proving the cleaner fix. 2.10.2
finishes the sweep:
...
| [`idempotency`](src/idempotency/idempotencyPlugin.ts) | `onSend` | **`preSerialization`** | Needs payload + sets `X-Idempotency-Key` header; closes race on async `store.set` / `store.unlock` |
...
**Contributor invariant (new CLAUDE.md gotcha):** arc plugins set response
headers at `onRequest` (if derivable from request) or `preSerialization`
(if payload-dependent). `onSend` is reserved for diagnostic tracing and
final byte-level transforms — never for header mutation.
...
### Pagination fix (original 2.10.2)
