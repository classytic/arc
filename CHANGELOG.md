...
| Version | Headline |
|---|---|
| [2.37](changelog/v2.md#2370) | `schedulesPlugin` `drainTimeoutMs` bounds shutdown drain (default 5 s). Perf tests ported to Node 24's GC schedule. [2.37.1](changelog/v2.md#2371): ⚠ `hidden: true` fields were returned whenever the client sent no `select` (and when a select blocked every field). Better Auth team scope is active-org-safe and identical for required/optional auth; ⚠ `better-auth` floor `>=1.7.0`. |
...
| [2.33](changelog/v2.md#2330) | **Write verbs** + **transactional write envelope** (`transactional: true`) — bind a CRUD slot to a domain command, pipeline intact. Outbox published NOTHING through its default transport. ⚠ repo-core floor >=0.24.0, primitives >=0.23.0. ⚠ `NODE_ENV=prod` disclosed stack traces and dropped `Secure` cookies. |
