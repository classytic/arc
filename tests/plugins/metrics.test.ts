...
describe('Metrics Plugin', () => {
  ...
  describe('HTTP request metrics', () => {
    it('should track request count and duration', async () => {
      ...
      const requestTotal = metrics.find(
        (m: MetricEntry) => m.name === 'arc_http_requests_total',
      );
  ...
  describe('CRUD operation metrics', () => {
    it('should expose increment/observe methods for resource operations', async () => {
      ...
      const metrics = app.metrics.collect();
      const opTotal = metrics.find(
        (m: MetricEntry) => m.name === 'arc_crud_operations_total',
      );
  ...
  describe('Cache metrics', () => {
    it('should track cache hits and misses', async () => {
      ...
      const metrics = app.metrics.collect();
      const cacheHits = metrics.find(
        (m: MetricEntry) => m.name === 'arc_cache_hits_total',
      );
      const cacheMisses = metrics.find(
        (m: MetricEntry) => m.name === 'arc_cache_misses_total',
      );
  ...
  describe('Event metrics', () => {
    it('should track event publish and consume counts', async () => {
      ...
      const metrics = app.metrics.collect();
      const published = metrics.find(
        (m: MetricEntry) => m.name === 'arc_events_published_total',
      );
      const consumed = metrics.find(
        (m: MetricEntry) => m.name === 'arc_events_consumed_total',
      );
      ...
      const pubEntry = published!.values.find(
        (v: { labels: Record<string, string> }) =>
          v.labels.event_type === 'product.created',
      );
  ...
  describe('Circuit breaker metrics', () => {
    it('should track circuit breaker state changes', async () => {
      ...
      const metrics = app.metrics.collect();
      const cbState = metrics.find(
        (m: MetricEntry) => m.name === 'arc_circuit_breaker_state',
      );
  ...
  describe('Reset', () => {
    it('should clear all metrics on reset', async () => {
      ...
      const total = metrics.reduce(
        (sum: number, m: MetricEntry) => sum + m.values.length,
        0,
      );
      expect(total).toBe(0);
    });
