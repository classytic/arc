...
describe('Auth Token Type Enforcement', () => {
  ...
  async function createApp(opts: Record<string, unknown> = {}, extraRoutes?: (instance: FastifyInstance) => void) {
  ...
  describe('verifyRefreshToken()', () => {
    it('should accept a valid refresh token', async () => {
      ...
      const tokens = app.auth.issueTokens({ id: 'user-1' });
      const decoded = app.auth.verifyRefreshToken<{ id: string; type: string }>(tokens.refreshToken!);
