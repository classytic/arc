...
describe("Auto-Audit via Hook System", () => {
  ...
  it("auto-logs create operations", async () => {
    ...
    const createEntry = entries.find((e) => e.action === "create" && e.resource === "product");
  ...
  it("auto-logs update operations with before/after", async () => {
    ...
    const entries = store.getAll();
    const updateEntry = entries.find((e) => e.action === "update" && e.resource === "product");
  ...
  it("auto-logs delete operations", async () => {
    ...
    const entries = store.getAll();
    const deleteEntry = entries.find((e) => e.action === "delete" && e.resource === "product");
  ...
  it("captures user and org context from hook scope", async () => {
    ...
    await hookSystem.executeAfter(
      ...
      { _id: "o1", total: 99 },
      {
        ...
        context: { _scope: { kind: "member", organizationId: "org-xyz" } } as Record<
          string,
          unknown
        >,
      },
  ...
  it("handles custom action via manual fastify.audit.custom()", async () => {
    ...
    // Simulate manual audit from an additional route using `rawHandler`
    await (
  ...
  it("auto-audits pipeline `handler` routes that call controller methods (via hooks)", async () => {
    const store = new MemoryAuditStore();
    const hookSystem = new HookSystem({ logger: { error: () => {} } });
    ...
    // When a pipeline `handler` route calls controller.update(),
  ...
  it("auto-audit is no-op when plugin disabled", async () => {
    ...
    const audit = (
      app as unknown as { audit: Record<string, (...args: unknown[]) => Promise<void>> }
    ).audit;
