...
describe('Audit Trail', () => {
  ...
  describe('MemoryAuditStore', () => {
    ...
    it('should filter by organizationId', async () => {
      ...
      await store.log(createAuditEntry('product', 'p1', 'create', { user: { _id: 'u1' }, organizationId: 'org-a' }));
      await store.log(createAuditEntry('product', 'p2', 'create', { user: { _id: 'u1' }, organizationId: 'org-b' }));
    ...
    it('should support combined filters', async () => {
      ...
      await store.log(createAuditEntry('product', 'p1', 'create', { user: { _id: 'u1' }, organizationId: 'org-a' }));
      await store.log(createAuditEntry('product', 'p2', 'update', { user: { _id: 'u1' }, organizationId: 'org-a' }));
      await store.log(createAuditEntry('order', 'o1', 'create', { user: { _id: 'u2' }, organizationId: 'org-a' }));
  ...
  describe('userId extraction', () => {
    ...
    it('should handle ObjectId-like objects via String()', async () => {
      ...
      const entry = createAuditEntry('r', '1', 'create', { user: { _id: objectId as unknown as string } });
    ...
    it('should prefer _id over id', async () => {
      const { createAuditEntry } = await import('../../src/audit/stores/interface.js');
      const entry = createAuditEntry('r', '1', 'create', { user: { _id: 'from-id', id: 'from-fallback' } });
  ...
  describe('change detection', () => {
    it('should skip internal fields starting with _', async () => {
      const { createAuditEntry } = await import('../../src/audit/stores/interface.js');
      const entry = createAuditEntry('r', '1', 'update', { user: { _id: 'u1' } }, {
    ...
    it('should return undefined when no before state', async () => {
      const { createAuditEntry } = await import('../../src/audit/stores/interface.js');
      const entry = createAuditEntry('r', '1', 'create', { user: { _id: 'u1' } }, {
        after: { name: 'New' },
      });
