...
function createRedisMock(): RedisLike {
  ...
  return {
    ...
    duplicate: vi.fn(function (this: RedisLike) {
      return createRedisMock();
    }) as unknown as RedisLike["duplicate"],
...
describe("RedisEventTransport logger", () => {
  it("should use injected logger for synchronous handler errors", async () => {
    ...
    expect(logger.errorCalls.length).toBe(1);
    expect(logger.errorCalls[0]?.[0]).toContain(
      "[RedisEventTransport] Handler error for test.created",
    );
