...
describe('CLI docs command', () => {
  it('exports OpenAPI using Arc runtime response shapes', async () => {
    ...
    try {
      ...
      expect(listOp.responses['200'].content['application/json'].schema.properties).toHaveProperty('docs');
      expect(createOp.responses['201'].content['application/json'].schema.properties).toHaveProperty('data');
      expect(createOp.responses['201'].content['application/json'].schema.properties).not.toHaveProperty('doc');
      expect(getOp.responses['200'].content['application/json'].schema.properties).toHaveProperty('data');
      expect(getOp.responses['200'].content['application/json'].schema.properties).not.toHaveProperty('doc');
    } finally {
