...
function createMockRepo(overrides: Record<string, unknown> = {}) {
  return {
    getAll: vi.fn().mockResolvedValue({ docs: [], total: 0, page: 1, pages: 0, hasNext: false, hasPrev: false }),
...
describe('Bulk Preset', () => {
  ...
  describe('BaseController.bulkCreate', () => {
    it('calls repo.createMany and returns 201 with created items', async () => {
      ...
      const result = await controller.bulkCreate(await createReqCtx({ items: [{ name: 'A', price: 10 }, { name: 'B', price: 20 }] }));

      expect(result.success).toBe(true);
      expect(result.status).toBe(201);
      expect(result.data).toHaveLength(2);
      expect(result.meta).toEqual({ count: 2 });
      expect(repo.createMany).toHaveBeenCalledWith([{ name: 'A', price: 10 }, { name: 'B', price: 20 }]);
    });
  ...
  describe('BaseController.bulkUpdate', () => {
    it('calls repo.updateMany with filter and data', async () => {
      ...
      const result = await controller.bulkUpdate(
        await createReqCtx({ filter: { status: 'draft' }, data: { $set: { status: 'published' } } }),
      );

      expect(result.success).toBe(true);
      expect(result.status).toBe(200);
      expect(result.data).toEqual({ matchedCount: 5, modifiedCount: 5 });
      expect(repo.updateMany).toHaveBeenCalledWith({ status: 'draft' }, { $set: { status: 'published' } });
    });
    ...
    it('returns 400 when filter is empty', async () => {
      ...
      const result = await controller.bulkUpdate(await createReqCtx({ filter: {}, data: { name: 'x' } }));
    ...
    it('returns 400 when data is empty', async () => {
      ...
      const result = await controller.bulkUpdate(await createReqCtx({ filter: { active: true }, data: {} }));
    ...
    it('returns 501 when repo lacks updateMany', async () => {
      ...
      const result = await controller.bulkUpdate(await createReqCtx({ filter: { a: 1 }, data: { b: 2 } }));
  ...
  describe('BaseController.bulkDelete', () => {
    it('calls repo.deleteMany with filter', async () => {
      ...
      const controller = new BaseController(repo, { resourceName: 'product' });
      const result = await controller.bulkDelete(await createReqCtx({ filter: { archived: true } }));
