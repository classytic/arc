...
describe('Per-Tenant Rate Limit — Integration', () => {
    ...
    app = await createApp({
      ...
      rateLimit: {
        ...
        keyGenerator: (request: unknown) => {
          const req = request as { ip: string; scope?: { kind: string; organizationId?: string; userId?: string } };
          return keyGen({
            ip: req.ip,
  ...
  it('generates different keys for different scope kinds', async () => {
    ...
    expect(keyGen({
      ip: '1.1.1.1',
      scope: { kind: 'member', userId: 'u1', userRoles: [], organizationId: 'org-1', orgRoles: [] },
    })).toBe('org-1');
