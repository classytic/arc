...
describe('Compensation — Flexibility', () => {
  beforeAll(async () => {
    ...
    ProductModel = connection.model<IProduct>('CompFlexProduct', new Schema<IProduct>({
  ...
  describe('in custom BaseController', () => {
    it('compensation works inside a controller method called via additionalRoute', async () => {
      ...
      class ProductController extends BaseController<IProduct> {
        async publishProduct(req: { params: Record<string, string>; body: unknown; query: unknown; headers: unknown; metadata: unknown }) {
      ...
      class ProductController extends BaseController<IProduct> {
        async publishProduct(req: { params: Record<string, string>; body: unknown; query: unknown; headers: unknown; metadata: unknown }) {
          ...
          const result = await withCompensation('publish-fail', [
            ...
            {
              name: 'external-call',
              execute: async () => { throw new Error('Search service down'); },
            },
          ]);
  ...
  describe('with Arc events via onStepComplete', () => {
    it('emits events for each completed step', async () => {
      ...
      await withCompensation('publish', [
  ...
  describe('fire-and-forget mixed with blocking', () => {
    it('non-blocking step does not delay subsequent steps', async () => {
      ...
      let bgResolve: () => void;
      const bgPromise = new Promise<void>((r) => { bgResolve = r; });

      const result = await withCompensation('mixed', [
        {
          name: 'db-write',
          execute: async () => { order.push('db'); return { written: true }; },
        },
        {
          name: 'send-analytics',
          execute: async () => { await bgPromise; order.push('analytics'); return {}; },
          fireAndForget: true,
        },
        {
          name: 'respond',
          execute: async () => { order.push('respond'); return { done: true }; },
        },
      ]);
  ...
  describe('standalone usage (no framework)', () => {
    it('works as a pure utility without any Arc dependency', async () => {
      ...
      const result = await withCompensation('pure', [
        { name: 'step1', execute: async () => { log.push('1'); return { a: 1 }; } },
        { name: 'step2', execute: async () => { log.push('2'); return { b: 2 }; } },
      ]);
