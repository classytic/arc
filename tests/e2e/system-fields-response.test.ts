 ...
 *    - List: { success, data, page, limit, total, pages, hasNext, hasPrev }
...
describe("System Fields & Response Format E2E", () => {
  ...
  it("POST /tasks returns 201 with correct response envelope", async () => {
    ...
    expect(body.createdBy).toBe(USER_A);
    expect(body.organizationId).toBe(ORG_A);

    createdItemId = body._id;
  });
  ...
  it("GET /tasks returns paginated envelope { success, data, page, limit, total, pages, hasNext, hasPrev }", async () => {
    const token = app.auth.issueTokens({
      id: USER_A,
      role: ["user"],
      organizationId: ORG_A,
    ...
    // body.data IS the array; no nested envelope.
  });
  ...
  it("PATCH /tasks/:id returns 200 with updatedBy auto-injected", async () => {
    ...
    // 2.13: update emits the doc directly (no envelope, no helper) — the body IS the doc.
    expect(body.updatedBy).toBe(USER_A);
  });
  ...
  it("system fields in update body are stripped", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
    expect(body.title).toBe("Should Update Title");
    // __v should not be 999
    expect(body.__v).not.toBe(999);
    // createdAt should not be year 2000
    expect(new Date(body.createdAt).getFullYear()).toBeGreaterThan(2020);
  });
...
describe("FieldRules E2E", () => {
  ...
  it("systemManaged field is stripped from create body", async () => {
    ...
    const doc = await ProductModel.findById(body._id).lean();
  ...
  it("readonly field is stripped from update body", async () => {
    ...
    expect(res.statusCode).toBe(200);
    const body = JSON.parse(res.body);
    expect(body.name).toBe("Updated Doohickey");
  ...
  it("systemManaged fields are blocked from select queries", async () => {
    ...
    body.data.forEach((doc: Record<string, unknown>) => {
  ...
  it("readonly field is visible in read responses", async () => {
    ...
    expect(body.sku).toBe("SKU-DEFAULT");
  });
});
...
describe("Auto-Injected Fields E2E", () => {
  ...
  it("different user updating sets their own updatedBy", async () => {
    ...
    expect(body.createdBy).toBe(USER_A); // Still original creator
    expect(body.updatedBy).toBe(USER_B); // Updated by new user
  });
...
describe("Ownership & Access Control Response Format", () => {
  ...
  it("owner can update their own resource", async () => {
    ...
    expect(updateRes.statusCode).toBe(200);
    expect(JSON.parse(updateRes.body).title).toBe("My Updated Doc");
  });
  ...
  it("superadmin can bypass ownership check", async () => {
    ...
    expect(updateRes.statusCode).toBe(200);
    expect(JSON.parse(updateRes.body).title).toBe("Admin Updated");
  });
});
