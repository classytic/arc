...
describe('Compensation + MongoKit Transaction', () => {
  beforeAll(async () => {
    ...
    OrderModel = connection.model<IOrder>(`CompTxnOrder_${suffix}`, new Schema<IOrder>({
      userId: String,
      items: [String],
      total: Number,
    ...
    InventoryModel = connection.model<IInventory>(`CompTxnInv_${suffix}`, new Schema<IInventory>({
  ...
  it('commits transaction and skips compensation on full success', { retry: 2 }, async () => {
    ...
    if (!result.success) {
      ...
      throw new Error(`Compensation failed: step=${result.failedStep} error=${result.error} compensationErrors=${JSON.stringify(result.compensationErrors)}`);
    }

    // Verify DB state
    const order = await OrderModel.findById(result.results['db-operations'] && (result.results['db-operations'] as { orderId: string }).orderId).lean();
