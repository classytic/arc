...
import { allowPublic, requireAuth, requireRoles, requireOwnership } from '../../src/permissions/index.js';
...
// Skip e2e integration tests - requires full auth/org setup (optional)
describe.skip('Integration: Fastify + Mongoose + Arc', () => {
  ...
  beforeAll(async () => {
    ...
    // Generate tokens
    adminToken = jwt.sign(
      { sub: adminUserId, email: 'admin@test.com', roles: ['admin'], organizations: [{ _id: org1Id, roles: ['admin'] }] },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
    userToken = jwt.sign(
      { sub: regularUserId, email: 'user@test.com', roles: ['user'], organizations: [{ _id: org1Id, roles: ['member'] }] },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
    guestToken = jwt.sign(
      { sub: new mongoose.Types.ObjectId().toString(), email: 'guest@test.com', roles: [] },
      JWT_SECRET,
      { expiresIn: '1h' }
    );
    ...
    const ArticleModel = mongoose.models.IntegrationArticle || mongoose.model('IntegrationArticle', articleSchema);

    // Create repository using MongoKit
    const { Repository, softDeletePlugin } = await import('@classytic/mongokit');
    ...
    app = await createApp({
      ...
      plugins: async (fastify) => {
        // Register org scope plugin for multi-tenant support
        await fastify.register(orgScopePlugin);

        // Register hooks
        fastify.arc.hooks.before('article', 'create', async (ctx) => {
          if (ctx.data?.title && !ctx.data.slug) {
            return {
              ...ctx.data,
              slug: ctx.data.title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, ''),
            };
          }
        });
  ...
  describe('Authorization & Permissions', () => {
    ...
    it('should deny non-owner from updating article', async () => {
      ...
      const otherUserToken = jwt.sign(
        { sub: otherUserId, email: 'other@test.com', roles: ['user'], organizations: [{ _id: org1Id, roles: ['member'] }] },
        JWT_SECRET
      );
  ...
  describe('Multi-Tenant Organization Scoping', () => {
    beforeEach(async () => {
      ...
      await ArticleModel.create([
        { title: 'Org1 Article 1', organizationId: org1Id, createdBy: adminUserId },
        { title: 'Org1 Article 2', organizationId: org1Id, createdBy: regularUserId },
        { title: 'Org2 Article 1', organizationId: org2Id, createdBy: new mongoose.Types.ObjectId() },
      ]);
    });
    ...
    it('should prevent access to articles from other organizations', async () => {
      // Get org2 article ID
      const ArticleModel = mongoose.model('IntegrationArticle');
      const org2Article = await ArticleModel.findOne({ organizationId: org2Id });
  ...
  describe('CRUD Operations', () => {
    it('should create, read, update, delete successfully', async () => {
      ...
      const createResponse = await app.inject({
        ...
        payload: { title: 'CRUD Test', content: 'Initial content', status: 'draft' },
      });
  ...
  describe('Query Features', () => {
    beforeEach(async () => {
      const ArticleModel = mongoose.model('IntegrationArticle');
      await ArticleModel.create([
        { title: 'A First', organizationId: org1Id, createdBy: adminUserId, status: 'published' },
        { title: 'B Second', organizationId: org1Id, createdBy: adminUserId, status: 'draft' },
        { title: 'C Third', organizationId: org1Id, createdBy: regularUserId, status: 'published' },
        { title: 'D Fourth', organizationId: org1Id, createdBy: regularUserId, status: 'archived' },
      ]);
    });
