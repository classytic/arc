...
describe('Populate with Select - E2E Integration', () => {
    ...
    const postSchema = new mongoose.Schema(
      {
        ...
        content: String,
        authorId: { type: mongoose.Schema.Types.ObjectId, ref: 'TestAuthor', required: true },
        organizationId: { type: mongoose.Schema.Types.ObjectId, index: true },
      },
      { timestamps: true }
    );
  ...
  describe('Multiple References', () => {
    ...
    beforeEach(async () => {
      ...
      CategoryModel = mongoose.models.TestCategory || mongoose.model('TestCategory', categorySchema);
      ...
      MultiRefPostModel = mongoose.models.TestMultiRefPost || mongoose.model('TestMultiRefPost', multiRefPostSchema);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle populate with filter/pagination together', async () => {
      ...
      await PostModel.create([
        { title: 'Post 2', content: 'Content 2', authorId: author?._id, organizationId: orgId },
        { title: 'Post 3', content: 'Content 3', authorId: author?._id, organizationId: orgId },
      ]);
  ...
  describe('QueryParser - populateOptions parsing', () => {
    ...
    it('should parse multiple populate options', () => {
      ...
      expect(parsed.populateOptions!.find((p) => p.path === 'categoryId')?.select).toBe('name slug');
    });
  });
});
