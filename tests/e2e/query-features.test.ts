...
describe("Basic list", () => {
  it("GET /articles returns all with pagination metadata", async () => {
    ...
    const body = res.json();
    expect(body.docs.length).toBe(5);
...
describe("Filters", () => {
  it("exact match: ?status=published", async () => {
    ...
    expect(body.docs.every((d: any) => d.status === "published")).toBe(true);
  });
  ...
  it("operator: ?views[gte]=3000", async () => {
    ...
    const body = res.json();
    expect(body.docs.length).toBe(3); // AI 5000, Healthy 3000, React 8000
    expect(body.docs.every((d: any) => d.views >= 3000)).toBe(true);
  });
  ...
  it("combined: ?status=published&views[gte]=5000", async () => {
    ...
    const body = res.json();
    expect(body.docs.length).toBe(2); // AI 5000, React 8000
  });
});
...
describe("Sort", () => {
  it("ascending: ?sort=views", async () => {
    ...
    const body = res.json();
    const views = body.docs.map((d: any) => d.views);
  ...
  it("descending: ?sort=-views", async () => {
    ...
    const res = await get("/articles?sort=-views");
    const body = res.json();
    expect(body.docs[0].title).toBe("React 20"); // 8000
    expect(body.docs[1].title).toBe("AI Revolution"); // 5000
  });
  ...
  it("multi-field: ?sort=status,-views", async () => {
    ...
    expect(body.docs[0].status).toBe("archived");
  });
});
...
describe("Offset pagination", () => {
  ...
  it("page 2: ?limit=2&page=2&sort=-views", async () => {
    ...
    expect(body.docs[0].title).toBe("Healthy Eating"); // 3000
  });
...
describe("Keyset pagination", () => {
  it("cursor-based with _id sort (no overlap)", async () => {
    ...
    expect(body1.docs.length).toBe(3);

    const lastId = body1.docs[2]._id;
    const page2 = await get(`/articles?limit=3&after=${lastId}`);
    expect(page2.statusCode).toBe(200);
    const body2 = page2.json();
    expect(body2.docs.length).toBe(2); // 5 total - 3 on page 1 = 2

    // No overlap
    const page1Ids = new Set(body1.docs.map((d: any) => d._id));
    for (const doc of body2.docs) {
...
describe("Populate (ref-based)", () => {
  it("simple: ?populate=tag", async () => {
    ...
    const body = res.json();
    const react = body.docs[0]; // React 20, highest views
  ...
  it("on single item: GET /articles/:id?populate=tag", async () => {
    ...
    const body = res.json();
    expect(body.data.tag).toBeDefined();
    expect(body.data.tag.name).toBe("Technology");
  });
});
...
describe("Lookup ($lookup join)", () => {
  it("join by slug: ?lookup[t][from]=qftags&...&[single]=true", async () => {
    ...
    const body = res.json();
    expect(body.docs.length).toBe(5);
    const react = body.docs.find((d: any) => d.title === "React 20");
...
describe("Combined queries", () => {
  ...
  it("filter + sort + lookup", async () => {
    ...
    const body = res.json();
    expect(body.docs.length).toBe(3);
    expect(body.docs[0].title).toBe("React 20");
    expect(body.docs[0].t.name).toBe("Technology");
  });
  ...
  it("filter + sort + select + populate + pagination (the works)", async () => {
    ...
    expect(body.docs.length).toBe(2);
    expect(body.docs[0].title).toBe("React 20");
    expect(body.docs[0].views).toBe(8000);
    expect(body.docs[0].tag.name).toBe("Technology");
    expect(body.docs[0].body).toBeUndefined(); // excluded by select
  });
});
