...
describe("applyPresets conflict detection", () => {
  it("should throw when two presets define the same method + path", () => {
    const presetA = presetWithRoutes("presetA", [{ method: "GET", path: "/stats" }]);
    const presetB = presetWithRoutes("presetB", [{ method: "GET", path: "/stats" }]);

    expect(() => applyPresets(minimalConfig("product"), [presetA, presetB])).toThrow(
      "preset conflicts",
    );
  });
  ...
  it("should not throw when presets define different routes", () => {
    const presetA = presetWithRoutes("presetA", [{ method: "GET", path: "/stats" }]);
    const presetB = presetWithRoutes("presetB", [{ method: "GET", path: "/export" }]);

    expect(() => applyPresets(minimalConfig("product"), [presetA, presetB])).not.toThrow();
  });

  it("should not throw when presets share a path but use different methods", () => {
    const presetA = presetWithRoutes("presetA", [{ method: "GET", path: "/archive" }]);
    const presetB = presetWithRoutes("presetB", [{ method: "POST", path: "/archive" }]);

    expect(() => applyPresets(minimalConfig("product"), [presetA, presetB])).not.toThrow();
  });

  it("should include resource name and both preset names in the error message", () => {
    const presetA = presetWithRoutes("alpha", [{ method: "DELETE", path: "/purge" }]);
    const presetB = presetWithRoutes("beta", [{ method: "DELETE", path: "/purge" }]);
