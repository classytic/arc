  ...
  describe('forward execution', () => {
    it('runs all steps in order and returns success', async () => {
      ...
      const result = await withCompensation('order-flow', [
        { name: 'step-1', execute: async () => { order.push('1'); return { reserved: true }; } },
        { name: 'step-2', execute: async () => { order.push('2'); return { charged: true }; } },
        { name: 'step-3', execute: async () => { order.push('3'); return { sent: true }; } },
      ]);
    ...
    it('passes mutable context between steps', async () => {
      ...
      const result = await withCompensation('ctx-test', [
        {
          name: 'create-order',
          execute: async (ctx) => { ctx.orderId = 'ord-123'; return { orderId: 'ord-123' }; },
        },
        {
          name: 'charge',
    ...
    it('accepts initial context', async () => {
      ...
      const result = await withCompensation('init-ctx', [
  ...
  describe('compensation on failure', () => {
      ...
      const result = await withCompensation('failing', [
        {
          ...
          compensate: async () => { compensated.push('unreserve'); },
        },
        {
          name: 'charge',
          execute: async () => ({ charged: true }),
          compensate: async () => { compensated.push('refund'); },
        },
        {
          name: 'ship',
          execute: async () => { throw new Error('Warehouse offline'); },
        },
      ]);
    ...
    it('first step failure means no compensation', async () => {
      ...
      const result = await withCompensation('first-fail', [
        { name: 'boom', execute: async () => { throw new Error('boom'); }, compensate: spy },
    ...
    it('steps without compensate are skipped in rollback', async () => {
      ...
      await withCompensation('partial', [
        { name: 's1', execute: async () => ({}), compensate: async () => { compensated.push('c1'); } },
        { name: 's2-no-comp', execute: async () => ({}) },
        { name: 's3-fails', execute: async () => { throw new Error('fail'); } },
      ]);
      ...
      await withCompensation('comp-data', [
        {
          ...
          compensate: async (_ctx, result) => { compensateData = result; },
        },
        { name: 'fail', execute: async () => { throw new Error('fail'); } },
      ]);
    ...
    it('collects compensation errors without stopping rollback', async () => {
      ...
      const result = await withCompensation('comp-errors', [
        { name: 's1', execute: async () => ({}), compensate: async () => { compensated.push('c1'); } },
        { name: 's2', execute: async () => ({}), compensate: async () => { throw new Error('comp-2 failed'); } },
        { name: 's3', execute: async () => { throw new Error('original'); } },
      ]);
    ...
    it('captures non-Error throws', async () => {
      ...
      const result = await withCompensation('string-throw', [
        { name: 'bad', execute: async () => { throw 'string error'; } },
      ]);
  ...
  describe('usage in Fastify additionalRoute', () => {
        ...
        const result = await withCompensation('checkout', [
          {
            name: 'reserve',
            execute: async () => ({ reserved: true }),
            compensate: async () => { compensated.push('unreserved'); },
          },
          {
            name: 'charge',
            execute: async () => { throw new Error('Card declined'); },
            compensate: async () => { compensated.push('refunded'); },
          },
        ]);
    ...
    it('works with async external service calls (mocked)', async () => {
      ...
      app.post('/order', async (request) => {
        ...
        const result = await withCompensation('order', [
          {
            name: 'inventory',
            execute: async (ctx) => {
