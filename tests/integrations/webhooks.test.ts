...
async function waitForDelivery(app: FastifyInstance, expectedCount: number, timeoutMs = 500) {
  await vi.waitFor(
    () => {
      const count = app.webhooks.deliveryLog().length;
      if (count < expectedCount)
        throw new Error(`Expected ${expectedCount} deliveries, got ${count}`);
    },
  ...
  describe("pattern matching", () => {
    ...
    it("supports multiple patterns on one subscription", async () => {
      ...
      await app.webhooks.register({
        id: "wh-1",
        url: "https://a.com",
        events: ["order.created", "invoice.sent"],
        secret: "s",
      });
  ...
  describe("WebhookStore contract", () => {
    ...
    it("removes subscription from store on unregister", async () => {
      ...
      const customStore = {
        ...
        getAll: vi
          .fn()
          .mockResolvedValue([{ id: "wh-del", url: "https://a.com", events: ["*"], secret: "s" }]),
        save: vi.fn().mockResolvedValue(undefined),
        remove: vi.fn().mockResolvedValue(undefined),
      };
  ...
  describe("store error resilience", () => {
    ...
    it("unregister propagates store.remove() errors to caller", async () => {
      ...
      const brokenStore = {
        ...
        getAll: vi
          .fn()
          .mockResolvedValue([{ id: "wh-1", url: "https://a.com", events: ["*"], secret: "s" }]),
        save: vi.fn().mockResolvedValue(undefined),
